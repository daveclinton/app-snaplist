...
export const Modal = React.forwardRef(
  (
    ...
    const renderHandleComponent = React.useCallback(
      ...
      [title, modal.dismiss, colorScheme],
    );
...
const ModalHeader = React.memo(
  ({
    ...
    dismiss,
    colorScheme,
  }: ModalHeaderProps & { colorScheme: any }) => {
    return (
      <>
        {title && (
          <View
            ...
            <View className="flex-1">
              <Text
                className={`text-center text-[16px] font-bold ${
                  colorScheme === 'dark' ? 'text-white' : 'text-[#26313D]'
                }`}
              >
                {title}
              </Text>
        ...
        <CloseButton close={dismiss} colorScheme={colorScheme} />
      </>
    );
  },
);
...
const CloseButton = ({
  ...
  return (
    ...
    >
      <Svg
        className={`${
          colorScheme === 'dark' ? 'fill-white' : 'fill-neutral-300'
        }`}
        width={24}
        height={24}
        fill="none"
